...
def extract_aadhaar_number(image_path, keyword):
    try:
        ...
        for img in images:
            ...
            # Define the Aadhaar card number pattern (12 digits with optional spaces)
            ...
            if all_words_found:
                ...
                if aadhaar_matches:
                    ...
                    break  # Stop searching if we found an Aadhaar number
    ...
    try:
        ...
        for filename in os.listdir(folder_path):
            ...
            if os.path.isfile(file_path):
                ...
                aadhaar_number = extract_aadhaar_number(file_path, keyword)
                if aadhaar_number:
                    print(f"Aadhaar Number: {aadhaar_number}")
                    return aadhaar_number
                else:
                    print("Aadhaar number not found in this file.")
        print("No Aadhaar number found in any file.")
        return None
    except Exception as e:
...
if __name__ == "__main__":
    while True:
        ...
        keyword = str(input("Enter the keyword: ")).strip()

        # Process the folder to find Aadhaar number
        aadhaar_number = process_folder(folder_path, keyword)

        if aadhaar_number:
            print(f"Found Aadhaar Number: {aadhaar_number}")
        else:
            print("Aadhaar number not found in any file.")
